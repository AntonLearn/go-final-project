// Package nextdate provides functionality for calculating the next task occurrence
// based on repetition rules (daily, weekly, monthly, yearly).
package nextdate
...
// CheckDate checks the task date and updates it according to the repeat rule if needed
func CheckDate(task *db.Task) error {
	nowDateStr := time.Now().Format(format.DateFormatTemplateYYYYMMDD)

	if task.Date == "" {
		task.Date = nowDateStr
		logger.Infof("Task date was empty, set to current date: %s", nowDateStr)
	}

	taskDate, err := time.Parse(format.DateFormatTemplateYYYYMMDD, task.Date)
	if err != nil {
		logger.Errorf("Failed to parse task date %s: %v", task.Date, err)
		return err
	}

	nowDate, err := time.Parse(format.DateFormatTemplateYYYYMMDD, nowDateStr)
	if err != nil {
		logger.Errorf("Failed to parse current date: %v", err)
		return err
	}
	...
	if firstDateMoreSecondDate(nowDate, taskDate) {
		if errors.Is(err, errEmpty) {
			task.Date = nowDateStr
			logger.Info("Empty repeat rule detected, using current date")
			return nil
		} else {
			if err != nil {
				return err
			...
			logger.Infof("Task date updated to next occurrence: %s", nextDateStr)
			return nil
		}
	}

	logger.Infof("Task date %s is in the future, no update needed", task.Date)
	return nil
}

// firstDateMoreSecondDate returns true if firstDate is after secondDate
...
// NextDate calculates the next date according to the repetition rule
func NextDate(now time.Time, dstart string, repeat string) (string, error) {
	...
	if repeat == "" {
		logger.Info("Empty repeat rule provided")
		return "", errEmpty
	}

	resultDate, err := time.Parse(format.DateFormatTemplateYYYYMMDD, dstart)
	if err != nil {
		logger.Errorf("Failed to parse start date %s: %v", dstart, err)
		return "", err
	}

	repeats := strings.Split(repeat, " ")
	logger.Infof("Processing repeat rule: %s", repeat)
	...
	case "y":
		...
		logger.Info("Applying yearly repetition rule")
		for {
	...
	case "d":
		...
		if err != nil {
			logger.Errorf("Invalid number format in 'd' rule: %s", repeats[1])
		...
		logger.Infof("Applying daily repetition rule with interval: %d days", num)
		for {
	...
	case "w":
		...
		logger.Info("Applying weekly repetition rule")
		...
		for weekday := range strings.SplitSeq(repeats[1], ",") {
			...
			if err != nil {
				logger.Errorf("Invalid weekday number: %s", weekday)
				return "", fmt.Errorf("%s: %w", errRuleParameter, err)
			}
	...
	case "m":
		...
		logger.Info("Applying monthly repetition rule")
		...
		for monthday := range strings.SplitSeq(repeats[1], ",") {
			...
			if err != nil {
				logger.Errorf("Invalid monthday format: %s", monthday)
				return "", fmt.Errorf("%s: %w", errRuleParameter, err)
			}
		...
		if len(repeats) > 2 {
			...
			for month := range strings.SplitSeq(repeats[2], ",") {
				...
				if err != nil {
					logger.Errorf("Invalid month format: %s", month)
					return "", fmt.Errorf("%s: %w", errRuleParameter, err)
				}
	...
	default:
		logger.Errorf("Unsupported repeat rule: %s", repeats[0])
