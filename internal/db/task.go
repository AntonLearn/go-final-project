...
func AddTask(task *Task) (int64, error) {
	result, err := config.DB.Connect.Exec(`
...
func GetTasks(search string) ([]*Task, error) {
	...
	if search != "" {
		...
		if err == nil {
			rows, err = config.DB.Connect.Query(`
...
func GetTask(id int) (*Task, error) {
	var task Task
	err := config.DB.Connect.QueryRow(`
		SELECT id, date, title, comment, repeat 
...
func DeleteTask(id int) error {
	_, err := config.DB.Connect.Exec(`DELETE FROM scheduler WHERE id = :id`, sql.Named("id", id))
	if err != nil {
